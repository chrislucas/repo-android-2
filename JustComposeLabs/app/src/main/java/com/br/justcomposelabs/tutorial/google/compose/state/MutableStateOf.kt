package com.br.justcomposelabs.tutorial.google.compose.state

/*
    Pesquisar a diferenca entre

    compose mutablestateof vs mutablestateflow



    https://proandroiddev.com/mutablestate-or-mutablestateflow-a-perspective-on-what-to-use-in-jetpack-compose-ccec0af7abbf
    https://medium.com/@gtxtreme/observing-state-mutablestate-vs-mutablestateflow-abaf73b36021
    https://towardsdev.com/state-wars-mutablestateflow-vs-mutablestateof-in-jetpack-compose-when-to-use-what-f403048d009b
    https://www.reddit.com/r/androiddev/comments/1dkyzbg/confused_about_when_to_use_mutablestateflow_vs/
 */
