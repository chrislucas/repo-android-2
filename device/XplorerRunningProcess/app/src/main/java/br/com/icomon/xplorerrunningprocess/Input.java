...
/*
  ...
  */
        injectKeyEventWithReflection(event);
    }


    private void injectKeyEventWithReflection(InputEvent event) {
        try {
            /**
             * {@link InputManager#getInstance}
             * */
            Method method = InputManager.class.getMethod("getInstance");
            InputManager inputManager = (InputManager) method.invoke(null);
            Class<? extends InputManager> c = inputManager.getClass();
            Field injectInputEventModeWaitForFinish = c.getField("INJECT_INPUT_EVENT_MODE_WAIT_FOR_FINISH");
            Method injectInputEvent = c.getMethod("injectInputEvent", InputEvent.class, Integer.TYPE);
            Class type = injectInputEventModeWaitForFinish.getType();
            if (type != null)
                Log.i("TYPE_ATTRIBUTE", type.toString());
            int n = injectInputEventModeWaitForFinish.getInt(null);
            boolean b = (Boolean) injectInputEvent.invoke(null, event, n);
            Log.i("INJECT_INPUT_EVENT", String.valueOf(b));
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException | NoSuchFieldException e) {
            String message = e.getMessage() == null ? "Não foi possível recuperar o erro" : e.getMessage();
            Log.e("EXCEPTION_INPUT", message);
        }
    }
    ...
    private void injectMotionEvent(int inputSource, int action, long when, float x, float y, float pressure) {
        ...
        MotionEvent event = MotionEvent.obtain(when
                , when
                , action
                , x
                , y
