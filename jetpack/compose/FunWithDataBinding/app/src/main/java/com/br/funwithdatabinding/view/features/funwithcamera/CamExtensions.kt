package com.br.funwithdatabinding.view.features.funwithcamera

/*
    TODO
    https://developer.android.com/media/camera/camera2
    https://developer.android.com/media/camera/camerax

    Camera extensions
    https://developer.android.com/media/camera/camera-extensions

    api camera jetpack compose
    https://medium.com/@deepugeorge2007travel/mastering-camerax-in-jetpack-compose-a-comprehensive-guide-for-android-developers-92ec3591a189
    https://developer.android.com/jetpack/androidx/releases/camera
    https://medium.com/@dheerubhadoria/capturing-images-from-camera-in-android-with-jetpack-compose-a-step-by-step-guide-64cd7f52e5de

    zxing android
    https://github.com/journeyapps/zxing-android-embedded

    android algorithm read barcode
    https://developers.google.com/ml-kit/vision/barcode-scanning/android

    algorithm to read barcode

    algorithm to read qr code
    https://en.wikipedia.org/wiki/QR_code
    https://smartengines.medium.com/qr-code-localization-the-important-recognition-step-that-has-been-neglected-8b3ed4e8037
    https://github.com/ErenKaymakci/Real-Time-QR-Detection-and-Decoding
    https://ivantay2003.medium.com/qr-code-demystify-2a5263ab136e
    https://stackoverflow.com/questions/5446421/encode-algorithm-qr-code
 */