package com.br.funwithdatabinding.view.features.tutorials.google.acessibility.scalingtext

/*
    A ideia é explorar soluções para criar layouts adaptáveis ao aumento do tamanho da fonte
    usando os recursos de acessibilidade do sistema operacional

    Text scaling
    https://support.google.com/accessibility/android/answer/12159181?hl=en#zippy=%2Cview

    Guide to Supporting Text Scaling
    https://docs.deque.com/devtools-mobile/2023.8.16/en/android-supports-dynamic-type

    Auto adjust text size to fit the layout Android
    https://stackoverflow.com/questions/53030480/auto-adjust-text-size-to-fit-the-layout-android/53030669


    Accessible Text Scaling for Android
    https://dev.to/sigute/accessible-text-scaling-for-android-1ham

    https://developer.android.com/develop/ui/views/text-and-emoji/autosizing-textview

    Making the most of TextView auto-sizing on Android
    https://medium.com/over-engineering/making-the-most-of-textview-auto-sizing-on-android-a39f71b6f3af

    Auto resizing - Edit Text Font size | how to auto resize font size according to text in android
    https://www.youtube.com/watch?v=9LTFmn_4Okk

    How to Create a Magic Text That Always Fits Its Container - Jetpack Compose Tutorial
    https://www.youtube.com/watch?v=ntlyrFw0F9U

    https://m3.material.io/foundations/writing/text-resizing

    Supporting/Adjusting text size on different screen sizes in Android
    https://medium.com/@kamranramzan098/supporting-adjusting-text-size-on-different-screen-sizes-in-android-a6ea82a219f5

        How to Implement an Accessible Font Size Adjustment Tool on Your Website
    https://www.allaccessible.org/how-to-implement-an-accessible-font-size-adjustment-tool-on-your-website/
 */