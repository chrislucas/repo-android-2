package com.br.funwithjetpackcompose.tutorials.google.stateincompose.tutorials

/*
    https://developer.android.com/develop/ui/compose/state?authuser=1
    https://developer.android.com/develop/ui/compose/state-saving

    medium
    https://medium.com/@seungbae2/jetpack-compose-remembersaveable-simplifying-state-persistence-for-seamless-user-experience-b2f721692228
    https://medium.com/@vitorhilario/entendendo-states-remember-e-remembersaveable-no-jetpack-compose-c08b9eae5d18

    rememberSaveable
    remember vs rememberSaveable
    https://outcomeschool.com/blog/remember-vs-remembersaveable

    SavedStateHandle
    https://developer.android.com/reference/androidx/lifecycle/SavedStateHandle
    https://developer.android.com/topic/libraries/architecture/viewmodel/viewmodel-savedstate
    https://developersbreach.com/savedstatehandle-viewmodel-android/

    https://medium.com/@sandeepkella23/what-is-savedstatehandle-in-android-306861b5c348
    https://medium.com/@ranjeet123/empowering-android-viewmodels-mastering-savedstatehandle-for-seamless-state-persistence-c1ea8af3ca08
    https://www.dhiwise.com/post/practical-guide-to-using-savedstatehandle-in-android

    Accessing screen arguments in ViewModels using SavedStateHandle
    https://www.sasikanth.dev/accessing-screen-arguments-in-viewmodels-using-savedstatehandle/
 */
