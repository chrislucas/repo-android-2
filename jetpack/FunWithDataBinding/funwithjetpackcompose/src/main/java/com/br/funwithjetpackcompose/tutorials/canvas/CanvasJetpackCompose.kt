package com.br.funwithjetpackcompose.tutorials.canvas

/*
    Graphics in Compose
    https://developer.android.com/develop/ui/compose/graphics/draw/overview

    Exploring Canvas in Jetpack Compose — Crafting Graphics, Animations, and Game Experiences
    https://proandroiddev.com/exploring-canvas-in-jetpack-compose-crafting-graphics-animations-and-game-experiences-b0aa31160bff


    Creating Custom Shapes in Jetpack Compose Using Draw Functions
    https://medium.com/@charles-raj/creating-custom-shapes-in-jetpack-compose-using-draw-functions-1235405a5586

    Jetpack Compose: começando com Canvas
    https://medium.com/ioasys-voices/jetpack-compose-come%C3%A7ando-com-canvas-cda3b8ff2e93

    Jetpack Compose Canvas
    https://medium.com/falabellatechnology/jetpack-compose-canvas-8aee73eab393

    Canvas in Jetpack Compose
    https://blog.kotlin-academy.com/canvas-in-jetpack-compose-c6e7b651fd9b

    Custom painter
    https://developer.android.com/develop/ui/compose/graphics/images/custompainter
 */
