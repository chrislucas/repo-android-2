package com.br.funwithworkmanager

/*
    TODO
    workmanager android
        WorkManager — Kotlin APIs
        https://medium.com/androiddevelopers/workmanager-kotlin-apis-a0fb9dfbfeb6

        https://developer.android.com/topic/libraries/architecture/workmanager?hl=pt-br
        https://developer.android.com/develop/background-work/background-tasks/persistent?hl=pt-br

        https://codelabs.developers.google.com/?text=workmanager

        Background Work with WorkManager
        https://developer.android.com/codelabs/basic-android-kotlin-compose-workmanager?hl=en#0

        Advanced WorkManager
        https://developer.android.com/codelabs/android-adv-workmanager?hl=en#0

        Android WorkManager — Visão Geral e implementação.
        https://medium.com/@spparks_/android-workmanager-vis%C3%A3o-geral-e-implementa%C3%A7%C3%A3o-37020eb148f4

        WorkManager — O que é? Como usar?
        https://medium.com/android-dev-br/workmanager-o-que-%C3%A9-como-usar-26f5b800984e

        Trabalho em segundo plano com WorkManager (Kotlin)
        https://developer.android.com/codelabs/android-workmanager?hl=pt-br#0

        https://github.com/android/codelab-android-workmanager

        https://developer.android.com/codelabs/basic-android-kotlin-compose-workmanager#0

 */
